...
async function updateProfil(req, res) {
  ...
  if (req.agent.cp !== cp && !req.agent.is_admin)
    return res.status(403).json({ error: 'Accès refusé' });
  const { is_reserve, familles_hab, agent_colors, habilitations, pause_figee, compteur_corrections, fetes_tracking, pause_figee_fia_mois, pause_figee_fia_done, demandes_conges, notifications_acquittees, roulement } = req.body;
  try {
    await pool.query(
