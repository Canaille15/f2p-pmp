// patch_bulletin_45_deroule_blocs_separes.js
// Correction fondamentale : traiter les deux blocs séparément.
// Bloc1 (texte avant séparateur) → candidats restreints aux mois 01-06
// Bloc2 (texte après séparateur) → candidats restreints aux mois 07-12
// Cela élimine presque toute ambiguïté : (Di,1) dans bloc1 → {Feb,Mar} seulement,
// (Di,1) dans bloc2 → {Nov} seulement. Sans ce filtrage, Oct/Nov/Dec polluaient les
// candidats des entrées de bloc1 et vice versa.
...
const newFn = `function parseDeroulePrevisionnel(text) {
  ...
  // Ordre des colonnes dans chaque bloc (depuis les en-têtes MM/AAAA)
  const detectOrdre = (t, moisSet) => {
    ...
    // Compléter avec les mois manquants du même bloc
  ...
  const ord1 = detectOrdre(text, MOIS_BLOC1); // ordre réel depuis le texte complet
  const ord2 = detectOrdre(text, MOIS_BLOC2);

  // Calendrier par bloc : (DayAbbr_DayNum) → candidats triés par ordre colonne
  ...
  // Validation du code (rejette abréviations de jours et codes inconnus)
  ...
  // Second passage : prises de nuit orphelines (RP/RU + CodeNuit sans day abbr)
  ...
  while ((mn2 = NUIT_ORPHAN_RE.exec(text)) !== null) {
    ...
    for (const mm2 of cands2) {
      ...
      if (existing && !existing.periodes.some(p => p.code_equipe === "N")) {
...
console.log('✅ App.jsx mis à jour : déroulé par blocs séparés (candidats restreints 01-06 / 07-12).');
