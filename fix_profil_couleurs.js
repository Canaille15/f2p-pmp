...
const old = `  const { is_reserve, familles_hab, agent_colors, habilitations, pause_figee, compteur_corrections, fetes_tracking, pause_figee_fia_mois, pause_figee_fia_done, demandes_conges, notifications_acquittees, roulement } = req.body;
  try {
    await pool.query(
...
const newCode = `  const { is_reserve, familles_hab, agent_colors } = req.body;
  try {
    await pool.query(
...
if(c.includes(old)) {
  ...
  console.log('OK - updateProfil simplifié');
} else {
