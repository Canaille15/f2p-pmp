...
if (localStorage.getItem("f2ppmp_version") !== DATA_VERSION) {
  // Première fois ou nouvelle version : ne pas effacer, juste noter la version
  ...
  periodes.forEach(p=>{
    ...
    if(filterF!=="PAR"){
      POSTES_PRCI_3x8.forEach(poste=>{
        ...
        const ags=agents.filter(a=>{const en=schedule[`${a.id}-${dateKey}`];return en&&(en.jsCode===jsCode||en.poste===poste.label)&&!(EQ[en.equipe]?.prive||EQ_COLORS[en.equipe]?.prive);});
...
// ─── DÉTECTION FAMILLE RÉSERVISTE ────────────────────────────────────────────
function detectFamillesReserviste(agentId, schedule) {
  const PRCI_CODES = ["PICCL-","PICCLO","PICCLX","PIADJ-","PIADJO","PIADJX",
    "PILNE-","PILNEO","PILNEX","PILNO-","PILNOO","PILNOX","PIVGD-","PIVGDO",
    "PILCL-","PILCLO","PILCLX","PIPA1J","PIPA2J","PIPA3J","PIDPXJ","PIASSJ",
    "SD%","F-PRCI","K-PRCI","A-PRCI","AFOPRCI","CAF","PPRCI"];
  const PAR_CODES = ["PAAC1-","PAAC1O","PAAC1X","PAAC2-","PAAC2O","PAAC2X",
    "PAACXX","PAASMJ","PAPAUJ","PADPXJ","K-PAR","AFO PAR","F-PAR"];
  let hasPRCI=false, hasPAR=false;
  Object.entries(schedule).forEach(([key,entry])=>{
    if(!key.startsWith(agentId+"-")) return;
    const js=(entry?.jsCode||"").replace(/^#/,"").replace(/\/$/,"");
    if(PRCI_CODES.includes(js)) hasPRCI=true;
    if(PAR_CODES.includes(js)) hasPAR=true;
  });
    ...
    <div style={{background:`linear-gradient(135deg,${fam?.color||"#1e293b"},#334155)`,borderRadius:16,padding:"18px 22px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",color:"#fff"}}>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,marginTop:3,flexWrap:"wrap"}}>
          {agent.immatriculation&&<span style={{background:"rgba(255,255,255,.2)",borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:800,fontFamily:"monospace",letterSpacing:1}}>CP {agent.immatriculation}</span>}
          <span style={{background:"rgba(255,255,255,.12)",borderRadius:6,padding:"2px 7px",fontSize:10,opacity:.9}}>{agent.grade}</span>
          <span style={{background:"rgba(255,255,255,.12)",borderRadius:6,padding:"2px 7px",fontSize:10,opacity:.9}}>{agent.poste}</span>
          <span style={{background:"rgba(255,255,255,.12)",borderRadius:6,padding:"2px 7px",fontSize:10,opacity:.9}}>{fam?.label}</span>
        </div>
        {/* Roulement + Réserviste inline dans le bandeau */}
        <div style={{display:"flex",alignItems:"center",gap:6,marginTop:6,flexWrap:"wrap"}}>
          {profile.roulement&&<span style={{background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.25)",borderRadius:7,padding:"2px 9px",fontSize:10,fontWeight:700,color:"#fff",display:"flex",alignItems:"center",gap:4}}>
            🔄 {profile.roulement}
          </span>}
          {profile.isReserve&&<span style={{background:"rgba(16,185,129,.25)",border:"1px solid rgba(16,185,129,.4)",borderRadius:7,padding:"2px 9px",fontSize:10,fontWeight:700,color:"#fff",display:"flex",alignItems:"center",gap:4}}>
            🛡️ Réserviste
            {profile.famillesHab&&<span style={{background:"rgba(255,255,255,.2)",borderRadius:5,padding:"1px 5px",fontSize:9}}>
              {profile.famillesHab==="BOTH"?"PRCI+PAR":profile.famillesHab}
            </span>}
          </span>}
          {profile.isReserve&&profile.habilitations&&Object.keys(profile.habilitations).length>0&&<span style={{background:"rgba(255,255,255,.1)",borderRadius:7,padding:"2px 8px",fontSize:9,color:"rgba(255,255,255,.7)"}}>
            {Object.values(profile.habilitations).filter(v=>v==="VALIDE").length} poste(s) validé(s)
          </span>}
        </div>
        <div style={{fontSize:10,marginTop:4,opacity:.6}}>{hasPin?"🔐 Planning protégé":"🔓 Aucun PIN"}</div>
      </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      ...
      <div style={{background:"#fff",border:`1.5px solid ${profile.isReserve?"#10b981":"#e2e8f0"}`,borderRadius:14,padding:"14px 16px"}}>
        ...
        {profile.isReserve?(<>
          ...
          {/* Sélection famille(s) habilitée(s) */}
          {(()=>{
            ...
            return <div style={{marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:7}}>
                ...
                {suggestion&&<span style={{fontSize:9,background:"#fef3c7",color:"#92400e",borderRadius:6,padding:"1px 7px",fontWeight:700}}>
                  💡 {suggestion==="BOTH"?"PRCI + PAR":suggestion} détecté
                </span>}
              </div>
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {OPTS.map(o=>{
                  const sel=current===o.k; const isSug=suggestion===o.k;
                  return <button key={o.k} onClick={()=>setProfile({famillesHab:sel?null:o.k})}
                    style={{border:`2px solid ${sel?o.color:isSug?o.bd:"#e2e8f0"}`,
                      background:sel?o.bg:isSug?"rgba(255,255,255,.5)":"#f8fafc",
                      color:sel?o.color:"#475569",borderRadius:9,
                      padding:"5px 13px",cursor:"pointer",fontSize:11,
                      fontWeight:sel?800:500,display:"flex",alignItems:"center",gap:4}}>
                    {sel&&<span>✓</span>}{o.l}{isSug&&!sel&&<span style={{fontSize:8,color:"#ca8a04"}}> ↗</span>}
                  </button>;
                })}
              </div>
              {current&&<div style={{fontSize:9,fontWeight:600,marginTop:5,
          ...
          <button onClick={()=>setShowHab(true)} style={{width:"100%",background:"#064e3b",color:"#fff",border:"none",borderRadius:8,padding:"7px 0",cursor:"pointer",fontSize:11,fontWeight:700}}>⚙️ Postes habilités ({nbHab})</button>
        </>):<div style={{fontSize:10,color:"#94a3b8",fontStyle:"italic"}}>Active pour indiquer tes habilitations.</div>}
      </div>
    </div>
    ...
    {calView==="semaine"&&<>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>
        {weekDates.map((dk,i)=>{
          ...
          const code=en?.equipe; const eq=EQ_COLORS[code]||EQ[code];
          ...
          return <div key={dk} style={{border:isToday?"2px solid #6366f1":"1.5px solid #e2e8f0",borderRadius:10,overflow:"hidden",background:bg,boxShadow:isToday?"0 0 0 3px #eef2ff":"none"}}>
            ...
            <div style={{padding:"5px 5px",minHeight:54,display:"flex",flexDirection:"column",gap:3}}>
              {en&&showData&&<>
                <span style={{display:"inline-flex",alignItems:"center",gap:3,background:eq?.bg||"#f1f5f9",color:eq?.tc||eq?.textColor||"#475569",borderRadius:14,padding:"2px 7px",fontSize:9,fontWeight:700}}>
      ...
      <div style={{background:"#fff",border:"1.5px solid #e2e8f0",borderRadius:14,overflow:"hidden"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,padding:6}}>
          ...
          {monthDates.map(dk=>{
            ...
            let bg=isWE?"#f8fafc":"#fff";
            if(en&&eq&&showData){
              if(!isPrive)bg=eq.bg||eq.color||"#fff";
              else if(code==="RP")bg="#dcfce7";
              else if(code==="RU"||code==="RQ")bg="#fef9c3";
              else bg=eq.bg||eq.color||"#fff";
            }
            return <div key={dk} style={{background:bg,border:isToday?"2px solid #6366f1":"1px solid #e2e8f0",borderRadius:8,padding:"4px 5px",minHeight:52,cursor:"pointer",position:"relative",boxShadow:isToday?"0 0 0 2px #eef2ff":"none"}}
              ...
              {code&&showData&&<div style={{fontSize:8,fontWeight:700,color:eq?.tc||eq?.textColor,background:eq?.bg||eq?.color,borderRadius:4,padding:"1px 4px",display:"inline-block"}}>{(eq?.label||code)?.slice(0,4)}</div>}
              {en&&!showData&&<span style={{fontSize:12}}>🔒</span>}
              {en?.jsCode&&en.jsCode!==code&&showData&&<div style={{fontSize:7,color:"#94a3b8",fontFamily:"monospace",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{en.jsCode}</div>}
            </div>;
          })}
        </div>
      </div>
...
export default function App(){
  ...
  const [departDates,setDepartDates]=usePersist("departDates",{}); // {agentId: "YYYY-MM-DD"}
  const [schedule,setSchedule]=usePersist("schedule",{
  "P50-2026-05-29": {
    "equipe": "M",
    "jsCode": "PIVGD-",
    "horaires": "06h10–14h02",
    "prive": false
  },
  "P07-2026-05-29": {
    "equipe": "M",
    "jsCode": "PIADJ-",
    "horaires": "06h15–14h07",
    "prive": false
  },
  "P02-2026-05-29": {
    "equipe": "M",
  ...
  "P37-2026-05-29": {
    "equipe": "J",
    "jsCode": "SD%",
    "horaires": "08h00–16h43",
    "prive": false
  },
  "P38-2026-05-29": {
    "equipe": "J",
    "jsCode": "PIPA1J",
    "horaires": "08h45–18h15",
    "prive": false
  },
  "P39-2026-05-29": {
    "equipe": "J",
    "jsCode": "PIPA3J",
    "horaires": "08h45–16h30",
    "prive": false
  },
  "P40-2026-05-29": {
    "equipe": "J",
    "jsCode": "F-PRCI",
    "horaires": "09h00–14h30",
    "prive": false
  },
  "P41-2026-05-29": {
    "equipe": "J",
  ...
  "P34-2026-05-29": {
    "equipe": "AM",
    "jsCode": "PIVGDO",
    "horaires": "13h52–21h29",
    "prive": false
  },
  "P09-2026-05-29": {
    "equipe": "AM",
    "jsCode": "PIADJO",
    "horaires": "14h05–21h59",
    "prive": false
  },
  "P04-2026-05-29": {
    "equipe": "AM",
    "jsCode": "PICCLO",
    "horaires": "14h15–22h17",
    "prive": false
  },
  "P49-2026-05-29": {
    "equipe": "AM",
  ...
  "P06-2026-05-29": {
    "equipe": "N",
    "jsCode": "PIADJX",
    "horaires": "21h57–06h17",
    "prive": false
  },
  "P01-2026-05-29": {
    "equipe": "N",
    "jsCode": "PICCLX",
    "horaires": "22h15–06h17",
    "prive": false
  },
  "P17-2026-05-29": {
    "equipe": "N",
  ...
  "P43-2026-05-29": {
    "equipe": "J",
    "jsCode": "AFO PAR",
    "horaires": "09h00–16h45",
    "prive": false
  },
  "R09-2026-05-29": {
    "equipe": "J",
    "jsCode": "K-PAR",
    "horaires": "09h00–17h45",
    "prive": false
  },
  "R05-2026-05-29": {
    "equipe": "J",
  ...
  // Nettoyage auto : supprime les agents absents depuis > 1 an (vérifié à chaque rendu)
  useMemo(()=>{
  ...
  const VIEWS=[{k:"global",l:"🏢 Vue globale"},{k:"personal",l:"👤 Mon agenda"}];

  return <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
    <style>{`*{box-sizing:border-box}button:hover{opacity:.85}details summary::-webkit-details-marker{display:none}`}</style>

    {/* HEADER */}
    <div style={{background:"#fff",borderBottom:"1.5px solid #e2e8f0",position:"sticky",top:0,zIndex:50}}>
      <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",gap:10,height:52,padding:"0 14px"}}>
        ...
        <div style={{display:"flex",gap:2,background:"#f1f5f9",borderRadius:9,padding:3}}>
          {VIEWS.map(({k,l})=>(
            <button key={k} onClick={()=>setView(k)} style={{border:"none",borderRadius:7,padding:"5px 11px",cursor:"pointer",background:view===k?"#fff":"transparent",color:view===k?"#1e293b":"#94a3b8",fontSize:11,fontWeight:view===k?700:400,boxShadow:view===k?"0 1px 4px rgba(0,0,0,.08)":"none",whiteSpace:"nowrap"}}>{l}</button>
          ))}
        </div>
                ...
                <div style={{maxHeight:150,overflowY:"auto"}}>
                  {rows.map(a=><button key={a.id} onClick={()=>{
                    if(currentUser&&a.id===currentUser.agent?.id){
                      // C'est moi — pas besoin de re-vérifier
                      setCurrentAgent(a);setProfileOpen(false);setProfileSearch("");
                    } else if(isAdmin){
                      // Admin peut voir tous les plannings
                      setCurrentAgent(a);setProfileOpen(false);setProfileSearch("");
                    } else {
                      // Autre agent — doit se connecter avec ses propres identifiants
...
{/* Modale : vérification identité pour accéder au planning d'un autre agent */}
{loginTarget&&(()=>{
  const fam=FAM[loginTarget.fam];
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.75)",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(6px)"}}>
    <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:380,boxShadow:"0 24px 60px rgba(0,0,0,.35)",overflow:"hidden"}}>
      <div style={{background:`linear-gradient(135deg,${fam?.color||"#1e293b"},#334155)`,padding:"20px 22px",textAlign:"center"}}>
        ...
        <div style={{color:"rgba(255,255,255,.6)",fontSize:12,marginTop:3}}>{loginTarget.prenom} {loginTarget.nom} · {loginTarget.grade}</div>
      </div>
      <div style={{padding:"20px 22px",display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"#eff6ff",borderRadius:10,padding:"9px 12px",fontSize:12,color:"#1e40af"}}>
          Seul(e) <strong>{loginTarget.prenom} {loginTarget.nom}</strong> peut accéder à son planning personnel.
        </div>
        <div>
          <label style={{fontSize:10,fontWeight:700,color:"#64748b",display:"block",marginBottom:5,letterSpacing:.5}}>MATRICULE S.N.C.F.</label>
          <input value={mat} onChange={e=>{setMat(e.target.value.toUpperCase());setErr("");}}
            placeholder={`Ex: ${loginTarget.immatriculation||"168401861B"}`}
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
          <div style={{fontSize:10,fontWeight:700,color:"#64748b",letterSpacing:.5}}>CODE PIN (5 chiffres)</div>
          <div style={{display:"flex",gap:8}}>
            {[0,1,2,3,4].map(i=>(
              <input key={i} ref={pinRefs[i]} type="password" inputMode="numeric" maxLength={1}
                ...
                style={{width:46,height:54,textAlign:"center",fontSize:22,fontWeight:800,
                  border:`2px solid ${err?"#ef4444":pin[i]?"#0891b2":"#e2e8f0"}`,
...
export default function App(){
  ...
  const [loginTarget,setLoginTarget]=useState(null); // Agent sur lequel on veut switcher
