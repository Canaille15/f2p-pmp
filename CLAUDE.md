...
- **10/07 — Écran de connexion (CP+PIN)** : refait avec un seul champ invisible superposé sur 4 cases visuelles (même technique que le changement de PIN existant, `PinModal`) au lieu de 4 `<input>` séparés qui se repassaient le focus. Corrige à la fois la touche Entrée qui ne validait pas la connexion, et le clavier virtuel mobile qui se fermait/rouvrait à chaque chiffre saisi (changement de focus entre champs). Auto-validation ajoutée dès le 4e chiffre tapé (comme `PinModal`).
